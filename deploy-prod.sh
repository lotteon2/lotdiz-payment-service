...
if ! kubectl get deployment payment-deployment &> /dev/null; then
  echo "Create new kubernetes deployment resources..."
  kubectl create -f ./deployment-prod.yml
else
  echo "Apply new docker image "
  kubectl set image deployments/payment-deployment payment-service=$ECR_REGISTRY/$AWS_ECR_REPOSITORY:$IMAGE_TAG --record
fi
...
if ! kubectl get deployment payment-deployment &> /dev/null; then
  ...
  if ! kubectl rollout status deployment/payment-deployment --watch=false | grep -q "successfully rolled out"; then
    echo "Rolling update has failed. Initiating rollback..."

    kubectl rollout undo deployment/payment-deployment
    kubectl rollout status deployment/payment-deployment --watch=true
    ...
    if ! kubectl get service payment-service &> /dev/null; then
